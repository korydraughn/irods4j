...
class TestIRODSMetadata {
	...
	void testAddSetRemoveMetadataForDataObject() throws IOException, IRODSException {
		...
		try {
			...
			try (IRODSDataObjectOutputStream out = new IRODSDataObjectOutputStream(conn.getRcComm(), logicalPath, true /* truncate */, false /* append */)) {
				assertTrue(out.isOpen());
			}

			// Show that the data object has no metadata attached to it.
			String query = String.format("select DATA_NAME where META_DATA_ATTR_NAME = '%s' and META_DATA_ATTR_VALUE = '%s'", attrName, attrValue);
...
//			assertTrue(IRODSQuery.executeGenQuery2(conn.getRcComm(), query).isEmpty());
			IRODSQuery.GenQuery1QueryArgs queryArgs = new IRODSQuery.GenQuery1QueryArgs();
...
//			assertTrue(IRODSQuery.executeGenQuery2(conn.getRcComm(), query).isEmpty());
	...
	void testAddSetRemoveMetadataForCollection() throws IOException, IRODSException {
		...
		String query = String.format("select COLL_NAME where META_COLL_ATTR_NAME = '%s' and META_COLL_ATTR_VALUE = '%s'", attrName, attrValue);
		assertTrue(IRODSQuery.executeGenQuery2(conn.getRcComm(), query).isEmpty());
...
//			assertTrue(IRODSQuery.executeGenQuery2(conn.getRcComm(), query).isEmpty());
	...
	void testAddSetRemoveMetadataForResource() throws IOException, IRODSException {
		...
		String attrName = "resource_attr_name";
		String attrValue = "resource_attr_value";

		// Show that the resource has no metadata attached to it.
		String query = String.format("select RESC_NAME where META_RESC_ATTR_NAME = '%s' and META_RESC_ATTR_VALUE = '%s'", attrName, attrValue);
		assertTrue(IRODSQuery.executeGenQuery2(conn.getRcComm(), query).isEmpty());
	...
	void testAddSetRemoveMetadataForUserOrGroup() throws IOException, IRODSException {
		String attrName = "user_attr_name";
		String attrValue = "user_attr_value";

		// Show that the user has no metadata attached to it.
		String query = String.format("select USER_NAME where META_USER_ATTR_NAME = '%s' and META_USER_ATTR_VALUE = '%s'", attrName, attrValue);
		assertTrue(IRODSQuery.executeGenQuery2(conn.getRcComm(), query).isEmpty());
