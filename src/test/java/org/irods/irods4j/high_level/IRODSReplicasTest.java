...
class IRODSReplicasTest {
	...
	void testSupportForSingleQuotesInLogicalPaths() throws Exception {
		var collection = '/' + String.join("/", zone, "home", username, "'replica'collectio'n");
		var dataObject = String.join("/", collection, "data'object.t'xt");
		...
		try {
			...
			var buffer = "the super important data".getBytes(StandardCharsets.UTF_8);
