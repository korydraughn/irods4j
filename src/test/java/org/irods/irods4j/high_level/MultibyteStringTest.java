...
public class MultibyteStringTest {
	...
	void testWritingToADataObjectWithMultibyteCharactersInItsName() throws Exception {
		var dataObject = '/' + String.join("/", zone, "home", username, "project-데이터_пример_संख्या_وثيقة_تجربة_東京_サンプル_текст_Überraschung_Énergie_456.iso");
		var data = "created a data object that has a name containing multibyte characters.".getBytes(StandardCharsets.UTF_8);
		...
		try {
			...
			try (var in = new IRODSDataObjectInputStream(conn.getRcComm(), dataObject)) {
